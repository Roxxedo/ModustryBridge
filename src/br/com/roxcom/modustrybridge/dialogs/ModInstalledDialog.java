...
public class ModInstalledDialog extends BaseDialog {
    private static final float bestWidth = 500f;
    private static final float padding = 30f;

    public ModInstalledDialog (String modName) {
        super("@mods.install.title", Core.scene.getStyle(DialogStyle.class));
        ...
        table.row();
        table.add("@mods.install.message")
                .wrap()
                .width(width)
                .pad(5f)
                .get().setAlignment(Align.center, Align.center);
        ...
        buttons.button("@mods.install.button", this::hide);
